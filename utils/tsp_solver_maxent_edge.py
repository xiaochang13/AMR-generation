...
from maxent_feature_v2 import subgraph_match_edge, extract_feature_v2
...
def gen_subgraph_rules(amr, i, rules, rules_match):
    ...
    for sub_str, translations in rules.iteritems():
        ...
        if bit[i] == True:
            matches = subgraph_match_edge(amr, sub)
            assert len(matches) > 0
            for trans, pos in translations[:int(sys.argv[1])]:
                new_rules.append((sub, trans, pos, matches[0]))
                covers |= matches[0][-1]
    print 'subgraph covers: %d total: %d' %(len(covers), len(amr.edges))
    return new_rules

def gen_rel_rules(amr, rel_trans):
    rules = []
    for e, edge in enumerate(amr.edges):
        bit = set([e,])
        if edge.label in rel_trans:
            for trans in rel_trans[edge.label]:
                rules.append((edge.label, [trans,], None, (e, bit)))
    return rules

def gen_concept_rules(amr, concept_trans):
    def get_entity_rule(amr, n, node):
        edge_labels = [amr.edges[x].label for x in node.v_edges]
        i = edge_labels.index('name')
        nn, nnode = node.get_child(i)
        trans = nnode.get_children_str()
        bit = set([node.c_edge, node.v_edges[i]]) | set(nnode.get_all_descendent_edge())
        next = set()
        for i, label in enumerate(edge_labels):
            subn, subnode = node.get_child(i)
            if label == 'wiki':
                bit.add(node.v_edges[i])
                bit = bit | set(subnode.get_all_descendent_edge())
            else:
                next.add(subn)
    ...
    def get_date_rule(amr, n, node):
        ...
        bit = set([node.c_edge,])
        bit |= set([e for i, e in enumerate(node.v_edges)])
        bit |= set([node.get_child(i)[1].c_edge for i, e in enumerate(node.v_edges)])
    ...
    def get_concept_rule(concept_trans, amr, cur, cur_node):
        for e in cur_node.v_edges:
            edge = amr.edges[e]
            key = cur_node.node_str() + ' ' + edge.label + ' ' + amr.nodes[edge.tail].node_str()
            if key in concept_trans:
                return (cur_node.node_str(), concept_trans[key], None, (cur, set([cur_node.c_edge, e,])))
        bit = set([cur_node.c_edge,])
        if cur_node.node_str() in concept_trans:
            trans = concept_trans[cur_node.node_str()]
        else:
            trans = [cur_node.node_str_nosuffix(),]
        return (cur_node.node_str(), trans, None, (cur, bit))
    ...
    while len(queue) > 0:
        ...
        if cur_node.is_entity():
            ...
            rules.append(get_concept_rule(concept_trans, amr, cur, cur_node))
            next = [cur_node.get_child(i)[0] for i in range(len(cur_node.v_edges))]
            subn_set = set(next)
...
def solve_by_tsp(amr, iii, rules, rules_match, concept_trans, rel_trans, model, LM):
    def model_score(rule1, rule2):
        feats = extract_feature_v2(amr, rule1, rule2, LM)
    ...
    matched.extend(gen_rel_rules(amr, rel_trans))
    print ' + relation rule number', len(matched)
    ...
    # from subgraph rules
    for i, mmmmmm in enumerate(matched):
        (sub, trans, pos, (n, bit)) = mmmmmm
        mark = len(row_string)
...
if __name__ == '__main__':
    ...
    concept_trans = collections.defaultdict(list)
    for line in open('verbalization-list-v1.06.txt','rU'):
        ...
        if items[0] != 'DO-NOT-VERBALIZE':
            ...
            root = items[i+1]
            if len(items) > i+2:
                concept_trans[' '.join(items[i+1:i+4])].append(trans)
            else:
                concept_trans[items[i+1]].append(trans)
    ...
    for i, amr in reader.read_amr('AMR-generation/dev/aligned_amr_nosharp'):
        if i in (7, 95, ) or len(sent[i]) > 50:
            continue
        ref.append([sent[i],])
        rst = solve_by_tsp(amr, i, rules, rules_match, concept_trans, rel_trans, LM)
        ans.append(rst)
