...
from maxent_feature import subgraph_match
from maxent_feature import extract_feature_v2 as extract_feature
...
def gen_concept_rules(amr, concept_trans):
    ...
    def get_concept_rule(concept_trans, amr, cur, cur_node):
        for e in cur_node.v_edges:
            ...
            if key in concept_trans:
                return (cur_node.node_str(), concept_trans[key], None, (cur, set([cur, edge.tail,])))
        bit = set([cur,])
    ...
    while len(queue) > 0:
        ...
        if cur_node.is_entity():
            ...
            rule = get_concept_rule(concept_trans, amr, cur, cur_node)
            rules.append(rule)
            next = [cur_node.get_child(i)[0] for i in range(len(cur_node.v_edges))]
            subn_set = set(next) - rule[-1][-1]
...
if __name__ == '__main__':
    ...
    model = maxent.MaxentModel()
    model.load('features.txt.balance.model')
    ...
    for i, amr in reader.read_amr('AMR-generation/dev/aligned_amr_nosharp'):
        if len(sent[i]) > 50:
            continue
        ref.append([sent[i],])
        rst = solve_by_tsp(amr, i, rules, rules_match, concept_trans, model)
        rst = [x.lower() for x in rst]
        ans.append(rst)
        sent_score = bleu_score.sentence_bleu(ref[-1], ans[-1])
        sent_length = 1.0*len(ans[-1])/len(ref[-1])
        if sent_score < 0.3 or sent_length < 0.7:
            ref.pop()
            ans.pop()
            print >>fid, i
        else:
